...
spark = SparkSession.builder \
    .appName("YoutubeChannelStreaming") \
...
df = spark.readStream \
    ...
    .option("subscribe", "youtube_channel_data") \
...
df_selected = df_selected.withColumn("view_count", col("view_count").cast("int")) \
       .withColumn("subscriber_count", col("subscriber_count").cast("int")) \
       .withColumn("video_count", col("video_count").cast("int"))

query = df_selected.writeStream \
    .partitionBy("year", "month", "day") \
    .format("parquet") \
    .option("path", "hdfs://namenode1:9000/youtube_DE_project/datalake/youtube_channel/raw/") \
    .option("checkpointLocation", "hdfs://namenode1:9000/youtube_DE_project/datalake/youtube_channel/raw_checkpoint/") \
    .outputMode("append") \
    .trigger(once=True) \
    .start()
